...
class SANERLAST(torch.optim.Optimizer):
    def __init__(self, params, rho=0.05, last_linear_param_ids=None, adaptive=False, alpha=1, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(SANERLAST, self).__init__(params, defaults)
        self.alpha = alpha
        self.last_linear_param_ids = last_linear_param_ids
    ...
    def second_step(self, zero_grad=False):
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if id(p) in self.last_linear_param_ids:
                    ratio = p.grad.div(param_state['first_grad'].add(1e-12))
                    mask = (ratio > 0) & (ratio < 1)
                    scale = torch.ones_like(ratio).masked_fill(mask, self.alpha)
