...
class CIFAR10Noisy_JO(torchvision.datasets.CIFAR10):
    def __init__(self, root, train=True, noise_type='symmetric', transform=None, target_transform=None, download=False, noise_rate=0.2, data_size=1):
        super(CIFAR10Noisy_JO, self).__init__(root, train=train, transform=transform, target_transform=target_transform, download=download)
...
def get_cifar10_jo(
    ...
    if use_val:
        data_train = CIFAR10Noisy_JO(root='./data', train=True, download=True, transform=transform_train, noise_rate=noise, noise_type=noise_type, data_size=data_size)
